...
int tecla()
{ // função que verifica se alguma tecla esta sendo precionada
    int varTecla;
    while (1)
    {
        if (kbhit())
        {                       // caso alguma tecla esteja sendo precionada a função irar armanezar o valor da tecla na variavel
...
void linha_Superior_Menu()
{ // função que cria linha superior do menu
    int i;
    printf("\t\t%c", 201);
    for (i = 0; i < STRAM; i++)
    {
...
void linha_Rodape_Menu()
{ // função que cria linha rodape do menu
    int i;
    printf("\t\t%c", 200);
    for (i = 0; i < STRAM; i++)
    {
...
void linha_Central_Menu()
{ // função que cria linha do meio do menu
    int i;
    printf("\t\t%c", 204);
    for (i = 0; i < STRAM; i++)
    {
...
void jNovamente_Linha_Cima()
{ // função que cria linha superior do menu de jogar novamente
    int i;
    printf("\t%c", 218);
    for (i = 0; i < STRAM2; i++)
    {
...
void jNovamente_Linha_Baixo()
{ // função que cria linha inferior do menu de jogar novamente
    int i;
    printf("\t%c", 192);
    for (i = 0; i < STRAM2; i++)
    {
...
void menu()
{ // função que faz o menu princial do jogo
    int c;
    int lugar = 0;
    ...
    { // laço de repetição infinito
        if (kbhit())
        { // se alguma tecla for precionada ele entrar dentro do if
            system("cls");
            ...
            { //apertou [enter]
                if (lugar == 0)
                { // se o enter(opçao de escolha) for apertado quando o menu estiver em start -  inicia o jogo
                    linha_Superior_Menu();
                ...
                { // se o enter(opçao de escolha) for apertado quando o menu estiver em regras - inicia a função regras
                    linha_Superior_Menu();
                    printf("\t\t%c            REGRAS            %C\n", 186, 186);
                    linha_Rodape_Menu();
                    regras();
                ...
                { // se o enter(opçao de escolha) for apertado quando o menu estiver em quit - inicia a função de sair
                    system("cls");
                    linha_Superior_Menu();
            ...
            if (lugar == 0)
            { // apos detectar a tecla apertada o programa imprime o local aonde a seta que demonstra a opçao de escolha esta
                linha_Superior_Menu();
                printf("\t\t%c            Jokenpo           %c\n", 186, 186);
                linha_Central_Menu();
                printf("\t\t%c          -> Start            %c\n", 186, 186);
...
void voltar_Menu()
{                             // função que volta ao menu
    char res_Egg, res_Regras; // declaração de variaveis

    printf("\n\nAperte esc para voltar ao menu");
    printf("\t\t\t\t\t\t\t\t-><-\n");
    res_Regras = getch();
    if (res_Regras != 27 && res_Regras != 13)
    {                  // se o botao apertado for diferente de ESC ou ENTER mostrar resposta invalida
    ...
    if (res_Regras == 27)
    {           // se o botao apertado for ESC
        menu(); // ele volta para o menu
    }
    if (res_Regras == 13)
    { // se o botao apertado for ENTRAR ele entrar no EASTER EGG
        system("cls");
...
void opcaoE()
{ // função que mostra as opções de escolha de utencilhos do jogo.
...
{ // função que opera o jogo.
    ...
    do
    {
        ...
        {
            if (espada_Jogador < 1)
            { // verifica se o jogador ja escolheu a opção da espada durante as 5 rodadas
                linha_Superior_Menu();
                printf("\t\t%c    Jogador escolheu ESPADA   %c\n", 186, 186);
            }
            espada_Jogador++; // espada do jogador recebe +1 no seu valor
            if (espada_Jogador > 1)
            { //Evitar que o jogador utilize a espada mais de 1 vez
        ...
        while (jogador == 0)
        { // loop para rafazer as escolhas do jogador caso seja invalida
        ...
        {
            if (espada_PC < 1)
            { // verifica se o computador ja escolheu a opção da espada durante as 5 rodadas
                printf("\t\t%c  Computador escolheu ESPADA  %c\n", 186, 186);
                linha_Rodape_Menu();
            }
            espada_PC++; // espada do computador recebe +1 no seu valor
            if (espada_PC > 1)
            {                   //Evitar que o computador utilize a espada mais de 1 vez
        ...
        while (computador == 0)
        { // loop para rafazer as escolhas do computador caso seja invalida
...
void resultado()
{ // função que mostra o resultado da partida
...
void easteregg()
{ // easter egg codigo com algumas alterações do codigo original (https://gist.github.com/gcr/1075131)
...
{ //função principal
    ...
    do
    { //entrar em loop se o jogador escolher jogar novamente
